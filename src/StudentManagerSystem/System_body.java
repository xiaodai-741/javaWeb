...
import StudentManagerSystem.DAO.ManagerDAO;
...
public class System_body {
    ...
    List<Manager> man = ManagerDAO.findAll();
...
//		
    ...
    private void addManager(String name) {
        ...
        System.out.println("请输入您要添加的密码：");
        String password = sca.next();
        if (ManagerDAO.save(new Manager(mid, sname, password)) >= 0) {
            System.out.println("恭喜添加成功，是否继续添加:0\\1  0:继续  1:退出");
        } else {
            System.out.println("添加失败");
        }
    ...
    private void updateManager(String name) {
        ...
        for (Manager manager : man) {
            if (manager.getId() == id) {
                ...
                String password = sca.next();
                if (ManagerDAO.merge(new Manager(id, Mname, password)) < 0) {
                    System.out.println("修改失败");
                }
                updateResult(name);
            }
